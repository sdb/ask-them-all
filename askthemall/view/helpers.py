...
def hidden_anchor(name):
    ...
    return a(name=name, id=name, style=styles(
...
def add_css_from_file(filename):
    filepath = os.path.join(os.path.dirname(__file__), os.pardir, os.pardir, 'css', filename)
    try:
...
def format_datetime(dt):
    ...
    try:
        ...
        logger.exception(f"Error getting current locale")
