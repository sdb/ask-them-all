...
from askthemall.view.helpers import (
    add_css_from_file,
    format_datetime,
    hidden_anchor,
    js_scroll_to,
...
def render():
            ...
            with st.container(key="sidebar-outline"):
                ...
                with st.expander("Questions", expanded=True):
                    for interaction in view_model.current_chat.interactions:
                        if st.button(
                            interaction.question_as_title,
                            type="tertiary",
                            help=interaction.question_as_title,
                            use_container_width=True,
        ...
        with st.container(key="sidebar-chats"):
            ...
            with search_col1:
                search = st.text_input(
                    "Search",
                    value=view_model.search_filter,
                    placeholder="Search...",
                    key="search-chats",
    ...
    if view_model.current_chat:
        ...
        for interaction in view_model.current_chat.interactions:
            st.markdown(
                hidden_anchor(interaction.interaction_id), unsafe_allow_html=True
            )
