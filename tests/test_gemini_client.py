...
from askthemall.clients.gemini import GeminiClient, GeminiSession
...
def genai_chat_session_class_mock():
    with patch("askthemall.clients.gemini.GenaiChatSession") as chat_session_class_mock:
...
def test_gemini_client_id_property():
    client = GeminiClient("some_id", "some_model_name")
    assert client.id == "some_id"


def test_gemini_client_name_property_based_on_model_name():
    client = GeminiClient("some_id", "some_model_name")
    assert client.name == "Gemini (some_model_name)"


def test_gemini_client_name_property():
    client_no_name = GeminiClient("some_id", "some_model_name", name='Some name')
    assert client_no_name.name == "Some name"


def test_gemini_client_start_session(genai_chat_session_class_mock, generative_model_mock):
    client = GeminiClient("some_id", "some_model_name")
...
def test_gemini_client_restore_session_empty_interaction_data(genai_chat_session_class_mock, generative_model_mock):
    client = GeminiClient("some_id", "some_model_name")
...
def test_gemini_client_restore_session_with_interaction_data(some_interaction_data, genai_chat_session_class_mock,
